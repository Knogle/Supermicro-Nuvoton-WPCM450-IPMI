...
//	write_reg( i2c_adap->nr, 0x1, I2C_CON3_REG);
   	...
   	while( ( read_reg( i2c_adap->nr, I2C_CON_STATUS_REG ) & I2C_BB ) && ( retries-- > 0 ) )
	{
		...
		write_reg( i2c_adap->nr, 0x02, I2C_CON_STATUS_REG);
       		...
       		dev_warn( &i2c_adap->dev, "wb_init: Bus busy, cannot synchronize with the bus activity\n" );
    	...
    	if( retries <= 0 )
    	{
        	dev_warn( &i2c_adap->dev, "wb_init: Bus busy, cannot synchronize the  bus activity\n" );
//        	return( -EIO );
...
//	write_reg( i2c_adap->nr, 0x04|read_reg(i2c_adap->nr,I2C_CON1_REG), I2C_CON1_REG );
	...
	if( status & I2C_NEGACK )
	{
		...
		write_reg(i2c_adap->nr,read_reg(i2c_adap->nr, I2C_STATUS_REG)|I2C_NEGACK,I2C_STATUS_REG);
		/* Got NACK */
//		dev_err( &i2c_adap->dev, "send_slave_addr: NACK on slave address transmission\n" );
		dev_err( &i2c_adap->dev, "send_slave_addr: NACK on slave address transmission (SlaveAddr=%x)\n", addr << 1);
		return( -EREMOTEIO );
	}
...
//		dev_err( &i2c_adap->dev, "send_slave_addr: Arbitration lost on slave address transmission\n" );
		dev_err( &i2c_adap->dev, "send_slave_addr: Arbitration lost on slave address transmission (SlaveAddr=%x)\n", addr << 1);
		return( -EREMOTEIO );
	}
...
//	printk (KERN_DEBUG "pulse_period = %x\n",pulse_period);
	...
	{ 
		...
		dev_info(&i2c_adap->dev, "Generate Clock Pulses...\n");	
...
//		wb_init(i2c_adap);
	...
	{
		wb_disable_i2c_controller (i2c_adap);
//		dev_info (&i2c_adap->dev, "Creating Forced STOP Condition\n");
    ...
    switch( cmd )
    {
		...
		case I2C_SET_REC_INFO:
		{
			...
			if (0 != set_recovery_info (i2c_adap->nr, &bus_recovery_info)) {
				dev_err (&i2c_adap->dev, "wb_control: set recovery info failed\n");
			}
			break;
		}
		case I2C_GET_REC_INFO:
		{
			...
			printk ("chk1\n");
			if (0 != get_recovery_info (i2c_adap->nr, &bus_recovery_info)) {
				dev_err (&i2c_adap->dev, "wb_control: get recovery info failed\n");
			}
...
//								dev_info (&i2c_adap->dev, "I2C Bus Recovered\n");
        ...
        default:
            dev_err( &i2c_adap->dev, "algo_control: Unknown command\n" );
            retval = -ENOTTY;
    	}
...
MODULE_DESCRIPTION( "Faraday I2C Algorithm" );
MODULE_LICENSE( "GPL" );
