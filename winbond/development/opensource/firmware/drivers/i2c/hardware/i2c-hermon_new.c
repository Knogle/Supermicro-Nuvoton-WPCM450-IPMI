...
void i2c_hermon_init_interanl_slave_data (int bus)
{
	...
	hermon_slave_data_ptr[bus].SlaveRX_len = 0;
	hermon_slave_data_ptr[bus].SlaveRX_index = 0;
	hermon_slave_data_ptr[bus].slave_stop_flag = 0;
}
...
static int __init i2c_hermon_init( void )
{
	...
	*( unsigned long* )(WPCM_AIC_VA_BASE +0x84) = reg32 |0x4C000000;
	...
	i2c_hermon_reset( i );
        ...
        {
            /* Request irq */
            if( request_irq( irq[ i ], hermon_handler, 0, "i2c_hermon", NULL ) < 0 )
            {
                printk( KERN_ERR "Request for irq %d failed\n", irq[ i ] );
                release_mem_region( raw_io_base[ i ], HERMON_I2C_REGS_SIZE );
                iounmap( i2c_hermon_io_base[ i ] );
...
static int i2c_hermon_copy_data (int bus, bus_data_info_T* bus_data_info)
{
	...
	while (Retries)
	{
		...
		{
			if(((i2c_hermon_read_reg(bus, I2C_STATUS_REG)&0x02)  == I2C_MASTER) &&
				(i2c_hermon_read_reg(bus, I2C_STATUS_REG)&0x40) ==I2C_SDAST)
			{
				master = 1;
			}
			if((i2c_hermon_read_reg(bus, I2C_STATUS_REG)&0x02)  == I2C_BER)
			{
				retries=1;
			}
		...
		{
			/* Turn off START bit */
			i2c_hermon_write_reg( bus, 0xFE & i2c_hermon_read_reg(bus, I2C_CON1_REG) ,I2C_CON1_REG );
			printk("I2C:Unable to start \n" );
			return( -ERESTARTSYS );
		}


		gI2CBuf [bus].State = I2C_STATE_SLAVE_SENT;
		
		/* if not clear to send */
		if ((i2c_hermon_read_reg( bus, I2C_STATUS_REG)& 0x40) != I2C_SDAST)
			return( -EREMOTEIO );
	
		/* enable stall after address send */
		i2c_hermon_write_reg( bus, 0x80|i2c_hermon_read_reg(bus, I2C_CON1_REG), I2C_CON1_REG);
		/* turn on interrupt */
		//i2c_hermon_write_reg( bus, 0x04|i2c_hermon_read_reg(bus,I2C_CON1_REG), I2C_CON1_REG );
	
		/* Set target slave address and direction bit */
		//if( flags & I2C_M_RD )
		//	i2c_hermon_write_reg( bus, ( gI2CBuf [bus].SlaveAddr << 1 ) | 0x01, I2C_DATA_REG );
		//else
		 step [bus] = 0;
			i2c_hermon_write_reg( bus, ( gI2CBuf [bus].SlaveAddr << 1 )&0xfe, I2C_DATA_REG );
		
		udelay(100);
		/* turn on interrupt */
		i2c_hermon_write_reg( bus, 0x04|i2c_hermon_read_reg(bus,I2C_CON1_REG), I2C_CON1_REG );
		//printk("Going to Wait for interrupt=%d\n",bus);
...
static u32 i2c_hermon_wait_for_int( int bus )
{
	...
	else if (I2C_NACK_ERROR == gI2CBuf [bus].Error)
	{
		...
		/* Reset and Reinit I2C Port (Though it is not really neede, but doing it anyway to be safe */
...
static int i2cRX_slave_process(int bus)
	...
	unsigned long flags;
            ...
            status = i2c_hermon_read_reg( bus, I2C_STATUS_REG );

	spin_lock_irqsave( &hermon_slave_data_ptr[bus].data_lock, flags);
	
		if( status & I2C_BER )
	    {
			
			//dev_info( &i2c_adap->dev, "slave_process: Bus Error\n" );
			printk("I2C channel %d slave_process: Bus Error\n", bus);
			/* clear BER in status reg */
			i2c_hermon_write_reg(bus,i2c_hermon_read_reg(bus, I2C_STATUS_REG)|I2C_BER,I2C_STATUS_REG);
			return( -EREMOTEIO );
	    }
		...
		if (status == 0)
		{
			...
			printk("I2C channel %d slave_process: Spurious Interrupt\n", bus);
			return 0;
    }

		if ( status & I2C_NMATCH)	
    {
			//printk("NMATCH index = %d\n",hermon_slave_data_ptr[bus].SlaveRX_index);
			hermon_slave_data_ptr[bus].SlaveRX_data[hermon_slave_data_ptr[bus].SlaveRX_index] = i2c_hermon_read_reg( bus, I2C_DATA_REG );

			hermon_slave_data_ptr[bus].SlaveRX_index++;

			/* clear NMATCH in status reg */
			i2c_hermon_write_reg(bus,i2c_hermon_read_reg(bus, I2C_STATUS_REG)|I2C_NMATCH,I2C_STATUS_REG);
		}
		...
		if (status & I2C_SLVSTP)
        {
			...
			//printk("I2C channel %d slave_process: stop condition received\n", bus);

			/* clear SLVSTP in status reg */
			i2c_hermon_write_reg(bus,i2c_hermon_read_reg(bus, I2C_STATUS_REG)|I2C_SLVSTP,I2C_STATUS_REG);
			//printk(" st reg = 0x%x\n", read_reg(i2c_adap->nr, I2C_STATUS_REG));
			
			/* set slave-stop-flag */

			hermon_slave_data_ptr[bus].slave_stop_flag = 1;

			hermon_slave_data_ptr[bus].SlaveRX_len = hermon_slave_data_ptr[bus].SlaveRX_index;
		}
		spin_unlock_irqrestore( &hermon_slave_data_ptr[bus].data_lock, flags);
		return 0;
...
ProcessI2CInterrupt (int BusNum)
	...
	/* If if the interrupt is for slave mode */
	if ((i2c_hermon_read_reg(BusNum, I2C_STATUS_REG)&0x02)  == 0) 
		{
			//printk(" slave mode....\n");
	...
	{	
		...
		case I2C_STATE_SLAVE_SENT:
			...
			if (0 == gI2CBuf [BusNum].Status) 						{ gI2CBuf [BusNum].Error = I2C_TO_ERROR; 	return 0; }
			else if (gI2CBuf [BusNum].Status == WB_SIGNAL_RECEIVED)	{ gI2CBuf [BusNum].Error = I2C_NACK_ERROR;  return 0; }
			else if (gI2CBuf [BusNum].Status & I2C_NEGACK)				{ gI2CBuf [BusNum].Error = I2C_NACK_ERROR;  return 0; }
			else if (gI2CBuf [BusNum].Status & I2C_BER) 		{ gI2CBuf [BusNum].Error = I2C_BUS_ERROR; 	return 0; }
			else if (!(gI2CBuf [BusNum].Status & I2C_SDAST)&& !(gI2CBuf [BusNum].Status & I2C_STASTR)) { gI2CBuf [BusNum].Error = I2C_TO_ERROR; 	return 0; }
        
			gI2CBuf [BusNum].State 	 = I2C_STATE_DATA_SENT;
			gI2CBuf [BusNum].WriteIx = 0;
			/*Clear Stall after Start*/
			i2c_hermon_write_reg (BusNum, i2c_hermon_read_reg(BusNum, I2C_STATUS_REG)|0x08, I2C_STATUS_REG);
			...
			if (((gI2CBuf [BusNum].WriteIx + 1) >= gI2CBuf [BusNum].WriteCnt) && (0 == gI2CBuf [BusNum].ReadCnt))
			{
				...
				//i2c_hermon_write_reg (BusNum, FA_STOP_XMIT, I2C_CONTROL_REG );
                	}
		...
		case I2C_STATE_STOP_SENT:
        	...
        	else if (gI2CBuf [BusNum].Status & I2C_NEGACK)				
			{
		...
		case I2C_STATE_DATA_SENT:
			...
			else if (!(gI2CBuf [BusNum].Status & I2C_SDAST)) 			{ gI2CBuf [BusNum].Error = I2C_TO_ERROR; 	return 0; }
        
			if (gI2CBuf [BusNum].WriteCnt == gI2CBuf [BusNum].WriteIx)
			{
				...
				/* if not clear to send */
				if ((i2c_hermon_read_reg( BusNum, I2C_STATUS_REG)& 0x40) != I2C_SDAST)
            {
					gI2CBuf [BusNum].Error =	I2C_BUS_ERROR;
					return 0;
        ...
        else
        {
				...
				//i2c_hermon_write_reg (BusNum, FA_XMIT_RCV, I2C_CONTROL_REG);
        }
			break;
		...
		case I2C_STATE_DATA_READ:
			...
			if ((gI2CBuf [BusNum].ReadIx + 1) >= gI2CBuf [BusNum].ReadCnt)
			{
				...
				/* Send the ACK after this byte */
				i2c_hermon_write_reg( BusNum, i2c_hermon_read_reg(BusNum, I2C_CON1_REG) | 0x10, I2C_CON1_REG );
				//i2c_hermon_write_reg (BusNum, FA_STOP_RCV, I2C_CONTROL_REG);
			}
			else
			{
				gI2CBuf [BusNum].State 	= I2C_STATE_DATA_READ;
...
#endif
{
	...
	/* Clear  interrupts */
	*( unsigned long *)(WPCM_AIC_VA_BASE + 0x12C) = 1<<vectNum;


	/* Read bus status */
	//op_status[ bus ] = i2c_hermon_read_reg( bus, I2C_STATUS_REG );
	gI2CBuf [bus].Status = i2c_hermon_read_reg( bus, I2C_STATUS_REG );
	//printk(" !!!!!!!!!!handler status-reg = 0x%x\n", op_status[bus]);
